...
class CountDownLatchWorker implements Runnable {
	...
	public CountDownLatchWorker(int id, CountDownLatch latch) {
		this.latch = latch;
		this.id = id;
	...
	public void run() {
		doWork();
		latch.countDown();
		System.out.println("Doing work after CountDown");
	}
...
public class CountDownLatches {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 5; i++)
			executorService.execute(new CountDownLatchWorker(i,latch));
	
		try {
			latch.await();//task which calls await will wait till count reaches to zero
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
